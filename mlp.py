...
def trainMarkov(markov, sample, depth):
...
def generateText(markov, cap, issong, length = -1, seed = ""):
	if seed == "":
		while seed == "" or seed == "ENDLINE" or seed == "NEWLINE":
	...
	first, *rest = seed
	rest.append(nterm)
	generateText(markov, cap, issong, length - 1, rest)
...
if __name__ == '__main__':
	...
	parser.add_argument("-ml", "--markovlength", help="number of words used to predict next word")
	parser.add_argument("-s", "--seed", help="seed word for word generation")
	parser.add_argument("-g", "--generate", action="store_true", help="generate text")
	parser.add_argument("-ll", "--limitlength", help="limits the length of the generated text to number of words in argument")
	parser.add_argument("--song", action="store_true", help="heeds the tokens NEWLINE, NEWVERSE, ENDSONG")
	args = parser.parse_args()

	markov = {}

	if args.load:
		with open(args.load, 'rb') as picklefile:
			markov = pickle.load(picklefile)

	if args.train:
		with open(args.train, 'r') as file:
			ml = 1
			if args.markovlength:
				ml = args.markovlength
			trainMarkov(markov, file.read(), ml)
	...
	if args.generate:
		...
		if args.limitlength:
			length = args.length
		generateText(markov, True, args.song, length, seed)
